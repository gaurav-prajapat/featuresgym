...
try {
    $stmt = $conn->prepare("
        SELECT gym_id, name, address, city, state, status, cover_photo
        FROM gyms
        WHERE gym_id = ?
    ");
    $stmt->execute([$gym_id]);
...
<head>
    ...
    <title>Gym Bookings - FlexFit Admin</title>
    ...
    <div class="ml-0 lg:ml-64 p-4 sm:p-6 transition-all duration-200">
        <div class="flex justify-between items-center mb-6">
            <div class="flex items-center">
                ...
                <h1 class="text-2xl font-bold">Bookings for <?= htmlspecialchars($gym['name']) ?></h1>
            </div>
            <a href="add_booking.php?gym_id=<?= $gym_id ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center">
            ...
            <div class="h-40 bg-gray-700 relative">
                ...
                <div class="p-6 bg-gray-700 bg-opacity-50">
                    <div class="flex space-x-2">
                        ...
                        <a href="gym_members.php?gym_id=<?= $gym_id ?>" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center">
                ...
                <div class="bg-gray-800 rounded-xl shadow-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold"><?= number_format($stats['scheduled']) ?></h3>
                        </div>
                        <div class="bg-indigo-900 bg-opacity-50 p-3 rounded-full">
                            <i class="fas fa-calendar text-indigo-400 text-xl"></i>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-800 rounded-xl shadow-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold"><?= number_format($stats['completed']) ?></h3>
                        </div>
                ...
                <div class="bg-gray-800 rounded-xl shadow-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold"><?= number_format($stats['cancelled']) ?></h3>
                        </div>
                ...
                <div class="bg-gray-800 rounded-xl shadow-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold"><?= number_format($stats['unique_members']) ?></h3>
                        </div>
                                            ...
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-10 w-10">
                                                    <?php if (!empty($booking['profile_image'])): ?>
                                                        <img class="h-10 w-10 rounded-full object-cover" src="<?= htmlspecialchars($booking['profile_image']) ?>" alt="Profile image">
                                                    <?php else: ?>
                ...
                <div class="px-6 py-4 bg-gray-700 border-t border-gray-600">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex space-x-1">
                            <?php if ($page > 1): ?>
                                <a href="?gym_id=<?= $gym_id ?>&page=1&status=<?= urlencode($status) ?>&date_from=<?= urlencode($date_from) ?>&date_to=<?= urlencode($date_to) ?>&sort=<?= urlencode($sort) ?>&order=<?= urlencode($order) ?>" class="px-3 py-1 rounded-md bg-gray-600 text-white hover:bg-gray-500">
                                    <i class="fas fa-angle-double-left"></i>
                                </a>
                                <a href="?gym_id=<?= $gym_id ?>&page=<?= $page - 1 ?>&status=<?= urlencode($status) ?>&date_from=<?= urlencode($date_from) ?>&date_to=<?= urlencode($date_to) ?>&sort=<?= urlencode($sort) ?>&order=<?= urlencode($order) ?>" class="px-3 py-1 rounded-md bg-gray-600 text-white hover:bg-gray-500">
                            ...
                            ?>
                                <a href="?gym_id=<?= $gym_id ?>&page=<?= $i ?>&status=<?= urlencode($status) ?>&date_from=<?= urlencode($date_from) ?>&date_to=<?= urlencode($date_to) ?>&sort=<?= urlencode($sort) ?>&order=<?= urlencode($order) ?>" class="px-3 py-1 rounded-md <?= $i === $page ? 'bg-blue-600 text-white' : 'bg-gray-600 text-white hover:bg-gray-500' ?>">
                                    <?= $i ?>
                                </a>
                            <?php endfor; ?>
                            ...
                            <?php if ($page < $total_pages): ?>
                                ...
                                <a href="?gym_id=<?= $gym_id ?>&page=<?= $total_pages ?>&status=<?= urlencode($status) ?>&date_from=<?= urlencode($date_from) ?>&date_to=<?= urlencode($date_to) ?>&sort=<?= urlencode($sort) ?>&order=<?= urlencode($order) ?>" class="px-3 py-1 rounded-md bg-gray-600 text-white hover:bg-gray-500">
